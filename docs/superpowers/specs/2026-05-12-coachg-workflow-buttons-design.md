...
| 3 | **GHL API client** | `lib/ghl.ts` with token refresh + 401 retry + `lib/supabase.ts`. | MSW-mocked tests: workflows.list, enroll, expired token triggers refresh, 401 triggers refresh-and-retry, refresh failure surfaces. |
